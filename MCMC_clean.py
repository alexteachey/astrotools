...
plot_option = raw_input('Do you want to plot figures? y/n: ')
savefile_name = raw_input("What do you want to call your param_guess_matrix numpy file? ")
...
if datatype == 'r':
    ...
    # FOR DYNAMIC FILE HANDLING. Skipping this for streamlining.
    """for n in [1,3,4]:
...
jumpsize_col2 = 1.4 * np.amin([mad(np.log10(uber_column_dictionary[0]['col2'])), mad(np.log10(uber_column_dictionary[1]['col2'])), mad(np.log10(uber_column_dictionary[2]['col2']))])
...
for i in np.arange(0,max_it,1): #steps
    ...
    try:
        ...
        if i == 0:
            ...
            if datatype == 'r':
                start_params = np.array([start_col2, start_col6, start_col7]) #feeding rhostar into the interpolator
            ...
            param_guess_matrix = np.array([start_col2, start_col6, start_col7]) #col2 is in log10.
            param_full_matrix = param_guess_matrix
            ...
            for p in np.arange(0,uber_column_dictionary.shape[0],1): #for each of the planets (3 in your case)
                testpoint = np.array([10**start_params[0], start_params[1], start_params[2]])
            ...
            if gocode == 1:
                if i == 1:
                    start2 = np.random.normal(loc=param_guess_matrix[0], scale=jumpsize_col2) #should be in log space.
        ...
        print "last loglike = ", loglike_list[-1]            
        print "new_loglike = ", new_loglike
        print " "
        print " "
...
np.save(savefile_name, param_guess_matrix)
...
if plot_option == 'y':    
    ...
    ### USING ONLY ACCEPTED TRIALS
    for param in np.arange(0,param_number,1):
        postburn =int( max_it/10)
        n, bins, patches = P.hist(param_guess_matrix.T[param][postburn:], 50, alpha=0.3)
        P.ylim(0,np.amax(n)+100)
